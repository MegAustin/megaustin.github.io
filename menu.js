...
document.querySelectorAll('.performance-nav').forEach(function(button) {
  button.addEventListener('click', function() {
    const rect = document.getElementById("performance-header").getBoundingClientRect();
    closeHamburgerMenu();
    console.log(rect.top);
    window.scrollTo({top:rect.top-50, behavior: 'smooth'});
  });
});

document.querySelectorAll('.bio-nav').forEach(function(button) {
  button.addEventListener('click', function() {
    const rect = document.getElementById("bio-header").getBoundingClientRect();
    closeHamburgerMenu();
    console.log(rect.top);
    window.scrollTo({top:rect.top-50, behavior: 'smooth'});
  });
...
document.querySelectorAll('.contact-nav').forEach(function(button) {
  button.addEventListener('click', function() {
    const rect = document.getElementById("contact-header").getBoundingClientRect();
    closeHamburgerMenu();
    console.log(rect.top);
    window.scrollTo({top:rect.top-50, behavior: 'smooth'});
  });
